...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const injectedMaskHelpersSource = `
                ...
                function isShellStripPreservedMedia(node) {
                    var name = node && node.nodeName ? String(node.nodeName).toLowerCase() : '';
                    return name === 'video' || name === 'source' || name === 'track';
                }

                function hasShellStripKeepAttribute(node) {
                    if (!node || !node.attributes) return false;
                    for (var i = 0; i < node.attributes.length; i += 1) {
                        var attr = node.attributes[i];
                        if (attr && /^data-csc-shellstrip-/.test(attr.name)) return true;
                    }
                    return false;
                }
                ...
                function shouldPreserveShellStripSibling(node) {
                    return !!(node && node.classList && node.classList.contains('cocosEditBox')) ||
                        hasShellStripKeepAttribute(node) ||
                        isShellStripPreservedMedia(node);
                }

                function restoreShellStripHiddenDisplay(node) {
                    if (!node || !node.style || !node.getAttribute) return;
                    if (node.getAttribute('data-csc-shellstrip-hidden-display') !== '1') return;
                    node.style.removeProperty('display');
                    node.removeAttribute('data-csc-shellstrip-hidden-display');
                }
                ...
                function hideSiblingsAlongAncestorChain(node, shouldApply) {
                    ...
                    while (current && current !== document.body) {
                        ...
                        for (var i = 0; i < parent.children.length; i++) {
                            ...
                            if (shouldPreserveShellStripSibling(sibling)) {
                                restoreShellStripHiddenDisplay(sibling);
                            } else if (sibling.style.display !== 'none') {
                                sibling.setAttribute('data-csc-shellstrip-hidden-display', '1');
            ...
            (function() {
                ...
                function sanitizeGame() {
                    ...
                    function ensureStyle() {
                        ...
                        style.textContent =
                            'html, body { margin: 0 !important; overflow: hidden !important; background: #000 !important; height: 100% !important; }' +
                            'body *:not([' + keepAttr + ']):not(.cocosEditBox):not(video):not(source):not(track) { display: none !important; }' +
                    ...
                    function preserveStoryVideoElements() {
                        var videos = document.querySelectorAll('video');
                        for (var i = 0; i < videos.length; i++) {
                            markKeepChain(videos[i], keepAttr);
                            var current = videos[i];
                            while (current && current !== document.body) {
                                restoreShellStripHiddenDisplay(current);
                                current = current.parentElement;
                            }
                            var sources = videos[i].querySelectorAll('source, track');
                            for (var j = 0; j < sources.length; j++) {
                                markKeepChain(sources[j], keepAttr);
                            }
                        }
                    }
                ...
                function scheduleSubframeAudioApply(maxAttempts, waitForInitialized) {
                    ...
                    function tick() {
                        ...
                        var readyForStableStop = initialized || stateApplied;
                        if (applied && readyForStableStop && stateApplied) {
                        ...
                        var canStop = applied && (
                            !waitForInitialized ||
                            (readyForStableStop && stateApplied && Date.now() - subframeAudioApplyStableSince >= SUBFRAME_AUDIO_APPLY_STABLE_MS)
                        );
                        if (canStop) {
    ...
    script.textContent = `
        (function() {
            ...
            if (!isGamePage) {
                ...
                function processMasterData(pathname, data) {
                    try {
                        // 路徑可能帶數字後綴（如 getMasterData1、getMasterData2），表示分頁資料
                        var matched = pathname && pathname.match(/^\\/gg\\/(.+)\\/getMasterData\\d*$/);
                        var masterDataType = matched && matched[1];
                        if (!masterDataType) return;
                        if (!masterData[masterDataType]) {
